...
-- Add the correct check constraint that allows all the status values we want to use
ALTER TABLE public.applications ADD CONSTRAINT applications_status_check 
CHECK (status IN ('pending', 'reviewed', 'interviewed', 'hired', 'rejected'));

-- Now clear existing data and repopulate
...
-- Create applications for the jobs using the existing admin user
DO $$
DECLARE
    job_record RECORD;
    app_count INTEGER;
    i INTEGER;
    status_values TEXT[] := ARRAY['pending', 'reviewed', 'interviewed', 'hired', 'rejected'];
    status_value TEXT;
BEGIN
    ...
    FOR job_record IN SELECT id, title FROM public.jobs LOOP
        ...
        FOR i IN 1..app_count LOOP
            -- Randomly select a status value
            status_value := status_values[floor(random() * array_length(status_values, 1) + 1)];
